...
void computeDistance(workspace* w) { //Computes the distance matrix using the numeric navigation function
	int i,j; 
	int nbrs;  
...
int path_finder(workspace * w){  
 	...
 	if(nb_possible_ngbrs(w->dx,w->dy,w)==0)
 		return 1; //path could not be found
		...
		for(i=0;i<ngbrs;i++){
			...
			if(in_list(ngbrsx[i],ngbrsy[i],closedlistx, closedlisty)==1)
			 ...
			 || tentative_gscore<gscore[ngbrsx[i]][ngbrsy[i]]){
				...
				fscore[ngbrsx[i]][ngbrsy[i]] = 
					gscore[ngbrsx[i]][ngbrsy[i]]+heuristic_cost_estimate(ngbrsx[i],ngbrsy[i], w->dx, w->dy);
